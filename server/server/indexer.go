...
func NewIndexer(version string, config *ServerConfig) *Indexer {
	indexer := new(Indexer)
...
func (this *Indexer) Search(request *SearchRequest) *SearchResponse {
	...
	for i := start; i < end; i++ {
		...
		response.Hits[i-start] = respHit
	}
	if cursor.Offset != 0 || cursor.CompleteSize > len(response.Hits) {
