...
func TestSearchServer(t *testing.T) {
	...
	file05 := tmpDir.Path("docs/bla1/file05.txt")
	file06 := tmpDir.Path("docs/bla2/file06.txt")
	file07 := tmpDir.Path("docs/bla2/file07.txt")
	files := make([]string, 3)
	files[resp.AssertHitDocId(file05).Index] = file05
	files[resp.AssertHitDocId(file06).Index] = file06
	files[resp.AssertHitDocId(file07).Index] = file07

	resp.AssertHitDocId(file05).AssertBefore("EE").AssertAfter("").AssertPosition(2)
	resp.AssertHitDocId(file06).AssertBefore("FF").AssertAfter("").AssertPosition(2)
	resp.AssertHitDocId(file07).AssertBefore("GG").AssertAfter("").AssertPosition(2)
	resp.AssertNoCursor()

	resp.AssertHitDocId(files[0]).AssertIndex(0)
	resp.AssertHitDocId(files[1]).AssertIndex(1)
	resp.AssertHitDocId(files[2]).AssertIndex(2)

	resp = client.searchBounded("ABC", 0, 10, 2)
	if resp == nil {
		return
	}
	resp.AssertHitCount(2)
	resp.AssertHitDocId(files[0])
	resp.AssertHitDocId(files[1])
	resp.AssertCursor(0, 3)

	resp = client.searchBounded("ABC", 1, 10, 2)
	if resp == nil {
		return
	}
	resp.AssertHitCount(2)
	resp.AssertHitDocId(files[1])
	resp.AssertHitDocId(files[2])
	resp.AssertCursor(1, 3)
